...
5. **The `User-Agent` URL changes to the confirmed production path.** The final home is `https://tools.wordzite.com/content-checker` (note: `-checker`, not `-check` — decided 2026-09-16, after the current string was written). Update `lib/sitemap/constants.js` to `ContentCheckerBot/1.0 (+https://tools.wordzite.com/content-checker)`. The UA remains identification only, for client logs; it grants nothing and the documentation must say so explicitly. In the same pass, update the `/content-check` examples in `.env.example`, `src/config.js`, `src/lib/url.js`, and `src/app.js` doc comments to `/content-checker` — these are comments and examples, not behaviour (the real mount path is `BASE_PATH`), but a stale path in a code comment misleads the next reader.
...
- QA1 confirms the User-Agent constant reads `/content-checker` and that a grep for `content-check` (without `-er`) across `src/`, `lib/`, `bin/`, `views/`, and `.env.example` returns nothing.

 ...
 * handling, and origin-scoped Authorization forwarding Sprint 3 already
 * built apply here for free, with nothing to get subtly wrong a second
 * time. Credentials are a plaintext `{username, password}` object
 * supplied by the caller (requirement 12) — this module never touches
 * Sprint 2's storage or its decrypt helper, and imports neither.
 *
...
async function resolveTitles(items, options = {}) {
  ...
  const stagingFetch = createGuardedFetch({ ...sharedGuardedFetchOptions, auth });

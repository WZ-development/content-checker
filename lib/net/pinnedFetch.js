 ...
 * wrapFetchWithOutboundToken below, composed on top of this in
 * src/app.js rather than folded in here. Keeping them apart means a
 * test that injects its own fake fetchImpl (bypassing pinning entirely,
 * for a hermetic test with no real network) still exercises the exact
 * same token-injection path production uses — pinning and token
 * injection are independently testable instead of only provable
 * together.
 */
 ...
 * the result carries the outbound token header, via
 * mergeOutboundTokenHeader above. This is the ONE place the header gets
 * added (src/app.js calls this exactly once, wrapping whichever
 * fetchImpl it ends up using); neither lib/sitemap's crawler nor
 * lib/compare's title fetcher ever sets this header themselves — they
 * just call whatever fetchImpl they were handed, same as always.
 * Because httpClient.js's redirect loop calls fetchImpl fresh on every
 * hop, and this wrapping is beneath that loop, every request type
 * requirement 1 names (initial fetch, every child sitemap, every title
 * fetch, every redirect hop) carries it automatically. Returns
 * `fetchImpl` completely unwrapped when no token is configured —
 * requirement 2 — so "not configured" has zero runtime cost or
 * behavioural difference beyond the header's absence.
 */
function wrapFetchWithOutboundToken(fetchImpl, outboundToken) {

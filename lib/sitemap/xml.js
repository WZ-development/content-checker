 ...
 *
 * `contentType`, when supplied, is used only to strengthen the
 * non-XML-vs-malformed distinction (an HTML content-type with an
 * XML-shaped body is still treated as non-XML) — the body's actual shape
 * is always the deciding factor, since servers frequently mislabel it.
 */
function parseSitemapXml(xmlText, { contentType, sourceUrl } = {}) {
  ...
  if (trimmed === '' || !trimmed.startsWith('<') || looksLikeHtml || contentTypeSaysHtml) {
